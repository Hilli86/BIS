...
(function (global, factory) {
  ...
  class jscanify {
    ...
    findPaperContour(img) {
      ...
      /* BIS: Canny erwartet 1 Kanal (RGBA sonst fehlerhaft); Kontur nach MatVector.delete klonen. */
      if (img.channels && img.channels() === 1) {
        cv.Canny(img, imgGray, 50, 200);
      } else {
        const gray = new cv.Mat();
        cv.cvtColor(img, gray, cv.COLOR_RGBA2GRAY);
        cv.Canny(gray, imgGray, 50, 200);
        gray.delete();
      }
      ...
      /* BIS: Lücken im Blattrand (niedriger Kontrast zum Untergrund) schließen. */
      const mk = cv.getStructuringElement(cv.MORPH_RECT, new cv.Size(5, 5));
      const imgClosed = new cv.Mat();
      cv.morphologyEx(imgThresh, imgClosed, cv.MORPH_CLOSE, mk);
      mk.delete();
      imgThresh.delete();
      ...
      const maxContourRef =
        maxContourIndex >= 0 ?
          ...
          null;
      let maxContour = null;
      if (maxContourRef && typeof maxContourRef.clone === 'function') {
        maxContour = maxContourRef.clone();
      }
